...
### couldn't find: Marital_status 

attr(df$Age, "Description") <- "The age of the mother"

df <- df %>% mustate("nationality" = NationalityDicotomica)
...
attr(df$`Education Level`, 'Description') <- "The highest level of education that the mother had received at the time of pregnancy/study"

df <- df %>% mutate('sport' = Sport)
attr(df$sport) <- 'tracking the physical activity of the mother; whether the mother played a sport (during pregnancy)' # ???




df_tidy <- select(Age, education_level, nationality, postpartum_depression, employed, occupation, sport, sport)
